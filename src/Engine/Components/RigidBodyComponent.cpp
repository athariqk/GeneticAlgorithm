...
void RigidBodyComponent::OnInit() {
    ...
    b2Polygon dynamicBox = b2MakeBox(transform->width * transform->scale / 2, transform->height * transform->scale / 2);
...
void RigidBodyComponent::OnDraw() {
    if (Scene::Get()->GetGUI().debugMode) {
        ...
        const SDL_FRect rect = {x - transform->width * 0.5f * transform->scale,
                                y - transform->height * 0.5f * transform->scale, transform->width * transform->scale,
