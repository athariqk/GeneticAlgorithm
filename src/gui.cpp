...
void GUI::OnInit() {
	imgui = new ImGuiLayer(Scene::Get()->GetWindow().GetWindow());
}
...
void GUI::OnImGuiRender() {
	imgui->Begin(); {
		...
		if (const auto &nutrients(
				Scene::Get()->GetEntityManager().GetGroup(Scene::groupLabels::NutrientsGroup));
		...
		{
			if (ImGui::BeginPopupModal("Create a new species", nullptr,
				...
				if (ImGui::Button("Create", ImVec2(80, 25))) {
					if (sGenus.empty() || sEpithet.empty()) {
						...
						Scene::Get()->GetEnvironment().addSpeciesToEnvironment(
		...
		ImGui::Text("Mouse motion (X: %f, Y: %f)", xRel, yRel);
